...
from dreamspace.core.animation import (
    HeartbeatRhythm,
    BreathingRhythm,
    WaveRhythm,
    LinearRhythm,
    ContinuousLinearRhythm,
)
...
class DreamspaceNavigator:
    ...
    def __init__(
        ...
        server_url: str,
        initial_prompt: str,
        image_size: Tuple[int, int] = (2048, 1280),
        batch_size: int = 2,
        noise_magnitude: float = 0.17,
        bifurcation_step: int = 3,
        output_format: str = "jpeg",
        maximize_window: bool = False,
        interpolation_mode: bool = False,
        prompt2: str = None,
        output_dir: str = None,
        prompt_list: List[str] = None,
        latent_cookie: int = None,
        seed: int = None,
        model: str = "sd15_server",
    ):
        ...
        if self.maximize_window:
            ...
            print(
                f"🎨 Generation size: {self.image_width}x{self.image_height} (unchanged)"
            )
            print(
                f"🪟 Window size: {self.window_width}x{self.window_height} (fullscreen)"
            )
            ...
            print(
                f"🔍 Display scale: {display_scale:.2f}x (images will be scaled up for display)"
            )
        else:
            print(
                f"🖼️ Using image size for both generation and display: {self.image_width}x{self.image_height}"
            )
        ...
        print(
            f"✅ Navigator initialized: {self.image_width}x{self.image_height} → {self.window_width}x{self.window_height}"
        )
        ...
        if self.latent_cookie is not None:
            ...
            print(
                f"🍪 Using latent cookie {self.latent_cookie} for consistent composition"
            )
        ...
        if self.output_dir:
            print(
                f"📁 Output directory: {self.output_dir} (will be cleared on each batch)"
            )
        else:
            print(
                f"📁 No output directory specified (images will not be saved to disk)"
            )
    ...
    def save_images_to_directory_with_consecutive_numbering(
        self, start_frame_number: int = None
    ):
        ...
        try:
            ...
            if frames:
                ...
                print(
                    f"💾 Saved frames {start_num}-{start_num + len(frames) - 1} to {self.output_dir}"
                )
                return len(frames)
    ...
    def generate_multi_prompt_sequence(self):
        ...
        print(
            f"📊 {self.batch_size} frames per segment = {len(prompt_pairs) * self.batch_size} total frames"
        )
        ...
        if self.latent_cookie is not None:
            ...
            print(
                f"🍪 Using user-specified latent cookie {latent_cookie} for consistent composition across sequence"
            )
        else:
            latent_cookie = random.randint(1, 1000000)
            print(
                f"🍪 Generated latent cookie {latent_cookie} for consistent composition across sequence"
            )
        ...
        if self.seed is not None:
            ...
            print(
                f"🎲 Using user-specified seed {shared_seed} for consistent generation across all segments"
            )
        else:
            shared_seed = random.randint(0, 2**32 - 1)
            print(
                f"🎲 Generated shared seed {shared_seed} for consistent generation across all segments"
            )

        with tqdm(
            total=len(prompt_pairs), desc="Generating interpolations", unit="segment"
        ) as pbar:
            for i, (prompt1, prompt2) in enumerate(prompt_pairs):
                ...
                pbar.set_description(
                    f"Segment {i+1}/{len(prompt_pairs)}: '{prompt1[:20]}...' → '{prompt2[:20]}...'"
                )
                ...
                try:
                    ...
                    if frames:
                        ...
                        frames_saved = (
                            self.save_images_to_directory_with_consecutive_numbering(
                                self.total_frames_saved
                            )
                        )
        ...
        if all_frames:
            ...
            print(
                f"📊 Generated {self.total_frames_saved} total frames across {len(prompt_pairs)} segments"
            )
            ...
            if self.output_dir:
                ...
                print(
                    f"🎬 Ready for video creation: frame_000000.{self.output_format} to frame_{self.total_frames_saved-1:06d}.{self.output_format}"
                )

            # Set ALL frames as current frames for animation (not just the last batch)
            print(
                f"🎬 Setting up animation with all {len(all_frames)} frames from multi-prompt sequence"
            )
    ...
    def generate_initial_batch(self):
        ...
        if self.is_multi_prompt_mode:
            print(
                f"🌈 Generating multi-prompt interpolation sequence ({len(self.prompt_list)} prompts)..."
            )
            return self.generate_multi_prompt_sequence()
        elif self.interpolation_mode and self.prompt2:
            print(
                f"🌈 Generating initial interpolated animation batch ({self.batch_size} frames)..."
            )
            print(f"   '{self.initial_prompt}' → '{self.prompt2}'")
        else:
            print(
                f"🎬 Generating initial animation batch ({self.batch_size} frames)..."
            )

        def generate_initial():
            start_time = time.time()
            print(
                f"⏰ Initial batch start time: {time.strftime('%H:%M:%S', time.localtime(start_time))}"
            )
            ...
            print(
                f"⏱️ Initial batch generation time: {duration:.2f} seconds ({duration/60:.1f} minutes)"
            )
            print(
                f"📊 Initial batch rate: {self.batch_size/duration:.1f} images/second"
            )
        ...
        if self.interpolation_mode and self.prompt2:
            ...
            self.img_gen.animation_controller.set_rhythm_modulator(
                ContinuousLinearRhythm(speed=1.2)
            )
            print(
                "🎵 Set Continuous Linear rhythm for interpolated embeddings ping-pong animation"
            )
    ...
    def start_background_generation(self, prompt: str):
        ...
        def generate_batch():
            try:
                ...
                print(
                    f"⏰ Request start time: {time.strftime('%H:%M:%S', time.localtime(start_time))}"
                )
                ...
                if frames:
                    ...
                    print(
                        f"⏱️ Generation time: {duration:.2f} seconds ({duration/60:.1f} minutes)"
                    )
        ...
        if key == pygame.K_RIGHT:
            ...
            if len(self.current_effects) < 3:
                new_effect = random.choice(
                    [e for e in MORPHS if e not in self.current_effects]
                )
                self.current_effects.append(new_effect)
                print(f"✨ Added effect: '{new_effect}'")
                gen_prompt = (
                    self.current_prompt + ", " + ", ".join(self.current_effects)
                )
            else:
    ...
    def handle_control_key(self, key: int):
        ...
        if key == pygame.K_a:
            ...
            speeds = [4, 8, 12, 16, 24]
            current_idx = (
                speeds.index(self.animation_speed)
                if self.animation_speed in speeds
                else 0
            )
            ...
            self.img_gen.animation_controller.set_rhythm_modulator(
                HeartbeatRhythm(base_bpm=35)
            )

        elif key == pygame.K_2:
            self.img_gen.animation_controller.set_rhythm_modulator(
                BreathingRhythm(base_period=12.0)
            )

        elif key == pygame.K_3:
            self.img_gen.animation_controller.set_rhythm_modulator(
                WaveRhythm(base_interval=2.8, chaos=0.6)
            )

        elif key == pygame.K_4:
            self.img_gen.animation_controller.set_rhythm_modulator(
                HeartbeatRhythm(base_bpm=55)
            )

        elif key == pygame.K_5:
            self.img_gen.animation_controller.set_rhythm_modulator(
                LinearRhythm(interval=1.2)
            )

        elif key == pygame.K_6:
            self.img_gen.animation_controller.set_rhythm_modulator(
                ContinuousLinearRhythm(speed=2.5)
            )
    ...
    def run(self):
        ...
        print_welcome_message(
            self.server_url, (self.image_width, self.image_height), self.batch_size
        )
        ...
        while running:
            ...
            if status["has_frames"]:
                frame_info = (
                    f"Frame: {status['current_frame_index']}/{status['frame_count']}"
                )
...
def main():
    ...
    maximize_input = (
        input("Maximize window to fill screen? (y/n, default=y): ").strip().lower()
    )
    maximize_window = maximize_input != "n"

    # Ask user if they want to save images to a directory
    output_dir_input = input(
        "Save images to directory? (leave empty to skip): "
    ).strip()
...
def main_with_args():
    ...
    interpolation_mode = (
        getattr(args, "interpolation_mode", False)
        or getattr(args, "prompt2", None) is not None
        or getattr(args, "prompt_list", None) is not None
    )
    ...
    if interpolation_mode and not prompt2 and not prompt_list:
        print(
            "❌ Interpolation mode requires --prompt2 or --prompt-list to be specified"
        )
        return

    if prompt2 and prompt_list:
        print(
            "❌ Cannot specify both --prompt2 and --prompt-list. Choose one interpolation mode."
        )
        return
