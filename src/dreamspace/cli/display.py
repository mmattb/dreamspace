...
class ImageDisplay:
    ...
    def set_window_size_from_image(self, image: Image.Image):
        ...
        if not self.maximize:
            ...
            print(f"🪟 Window resized to image size: {self.window_width}x{self.window_height}")
    ...
    def draw_text_overlay(self, text: str, position: str = "bottom-left", color: Tuple[int, int, int] = (255, 255, 255)):
        ...
        if position == "bottom-left":
            ...
            x, y = self.window_width - text_width - margin, self.window_height - text_height - margin
        elif position == "top-right":
            x, y = self.window_width - text_width - margin, margin
        else:
            x, y = margin, self.window_height - text_height - margin  # Default to bottom-left
...
class FrameAnimator:
    ...
    def __init__(self, frames: List[Image.Image], fps: int = 24, loop_mode: LoopMode = LoopMode.PING_PONG):
    ...
    def _setup_frame_sequence(self):
        ...
        if self.loop_mode == LoopMode.FORWARD:
            ...
            if len(self.frames) == 1:
                ...
                backward = list(range(len(self.frames) - 2, 0, -1))  # Exclude endpoints to avoid duplication
