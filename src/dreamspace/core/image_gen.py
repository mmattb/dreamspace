...
class ImgGen:
    ...
    Example:
        ...
        >>> img_gen = ImgGen("kandinsky21_server", prompt="a surreal forest")
        >>>
    ...
    def __init__(
        self,
        backend: Union[str, ImgGenBackend] = "sd21_server",
        ...
        **backend_kwargs,
        ...
        Args:
            backend: Either a backend type string ("sd21_server", "sd15_server", "kandinsky21_server")
    ...
    def _create_backend(self, backend_type: str, **kwargs) -> ImgGenBackend:
        ...
        if backend_type == "sd_local":
            ...
            from ..backends.server.sd15_server_backend import (
            ...
            from ..backends.server.sd21_server_backend import (
            ...
            from ..backends.server.kandinsky21_server_backend import (
                Kandinsky21ServerBackend,
            )
    ...
    def gen(
        ...
        if num_images > 1:
            ...
            # Register result
            self.register(
                image, result.get("embeddings"), result.get("latents"), use_prompt
            )
    ...
    def gen_interpolated(self, alpha: float) -> Image.Image:
        ...
        if not self._interpolation_source or not self._interpolation_target:
            raise ValueError(
                "Interpolation targets not set. Call set_interpolation_targets() first."
            )

        if not (
            self._interpolation_source["embedding"] is not None
            and self._interpolation_target["embedding"] is not None
        ):
    ...
    def generate_interpolated_embeddings(
        self, prompt1: str, prompt2: str, batch_size: int, **kwargs
    ) -> List[Image.Image]:
        ...
        result = self.backend.generate_interpolated_embeddings(
            prompt1=prompt1, prompt2=prompt2, batch_size=batch_size, **params
        )
        ...
        if isinstance(result, dict):
            ...
            if images:
                self.register(
                    images[0], result.get("embeddings"), result.get("latents"), prompt1
                )
        else:
