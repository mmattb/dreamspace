...
class Kandinsky21ServerBackend(ImgGenBackend):
    ...
    def _calculate_sub_batch_size(self, total_batch_size: int, width: int, height: int) -> int:
        ...
        available_memory_gb = 5.0  # Very conservative for Kandinsky's dual-stage pipeline
        ...
        memory_per_image_mb = latent_megapixels * 4 * 2 * (6 + 6 + 4)  # Very conservative for Kandinsky
        ...
        sub_batch_size = max(1, min(sub_batch_size, 4))  # Never more than 4 per sub-batch for Kandinsky
        
        print(f"📊 Kandinsky 2.1 Memory heuristic for {width}x{height} ({megapixels:.1f}MP):")
        print(f"   Estimated {memory_per_image_gb:.2f}GB per image (Kandinsky is very memory-intensive)")
    ...
    def generate(self, prompt: str, batch_size: int, noise_magnitude: float, bifurcation_step: int, output_format: str = "pil", latent_cookie: Optional[int] = None, **kwargs) -> Dict[str, Any]:
        ...
        guidance_scale = kwargs.get('guidance_scale', 4.0)  # Kandinsky typically uses lower guidance
        height = kwargs.get('height', 768)  # Kandinsky 2.1 default resolution
        width = kwargs.get('width', 768)   # Kandinsky 2.1 default resolution
        num_inference_steps = kwargs.get('num_inference_steps', 100)  # Kandinsky typically uses more steps
    ...
    def _generate_interpolated_embeddings_with_sub_batching(self, prompt1: str, prompt2: str, 
        ...
        # For decoding, we can also sub-batch if needed - Kandinsky VAE is very memory intensive
        decode_sub_batch_size = min(sub_batch_size * 1, 2)  # Kandinsky VAE decode needs very small batches
        all_decoded_images = []
